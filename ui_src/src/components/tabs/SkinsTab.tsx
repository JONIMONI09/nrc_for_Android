...
import { convertFileSrc } from "../../services/bridge-service";
      ...
      const fetchRender = async () => {
        if (skin && skin.name) {
          try {
            ...
            if (isMounted) {
              if (localPath) {
                setStarlightRenderUrl(convertFileSrc(localPath as string));
              } else {
                console.warn(
...
export function SkinsTab() {
  ...
  const loadSkinData = useCallback(async () => {
    ...
    try {
      ...
      setSkinData(data as any);

      if ((data as any)?.properties) {
        const texturesProp = (data as any).properties.find(
  ...
  const loadLocalSkins = useCallback(async () => {
    ...
    try {
      const skins = await MinecraftSkinService.getAllSkins() as MinecraftSkin[];
    ...
    try {
      ...
      if (updatedSkin) {
        setLocalSkins((prevSkins) =>
          prevSkins.map((s) => (s.id === (updatedSkin as any).id ? (updatedSkin as any) : s)),
        );
        if (selectedLocalSkin?.id === (updatedSkin as any).id) {
          setSelectedLocalSkin(updatedSkin as any);
        }
  ...
  const addSkin = async (
    ...
    try {
      ...
      setLocalSkins((prevSkins) =>
        [...prevSkins, newSkin as any].sort((a, b) => (a as any).name.localeCompare((b as any).name)),
      );
      hideModal('add-skin-modal');
    } catch (err) {
