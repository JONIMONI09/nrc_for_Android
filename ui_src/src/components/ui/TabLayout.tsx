    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-wrap md:flex-nowrap">
          {search && (
            <div className="w-full md:w-auto flex-grow md:flex-grow-0 h-[42px] relative">
               <input
                  type="text"
                  value={search.value}
                  onChange={(e) => search.onChange(e.target.value)}
                  placeholder={search.placeholder || t('common.search')}
                  className="w-full h-full bg-white/10 border-2 border-white/20 rounded px-3 text-white lowercase"
               />
            </div>
          )}
          {actions && (
