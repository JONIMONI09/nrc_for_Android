  ...
  return (
    <>
      <div
        ref={navRef}
        className={cn(
          "flex flex-row md:flex-col items-center justify-around md:justify-start md:py-6 w-full md:w-24 backdrop-blur-lg pb-safe",
          className,
        )}
        style={{
      ...
      >        <div className="hidden md:block mb-12">
        ...
        <div className="flex-1 flex flex-row md:flex-col items-center justify-around md:justify-start w-full md:w-auto md:space-y-4 md:min-h-[400px] overflow-x-auto no-scrollbar py-2 md:py-0">
          {items.map((item) => (
            <div
              key={item.id}
              className="relative group nav-item flex-shrink-0 mx-2 md:mx-0"
              ref={(el) => (buttonRefs.current[item.id] = el)}
            >
              <NavButton
                icon={<Icon icon={item.icon} className="w-6 h-6 md:w-8 md:h-8" />}
