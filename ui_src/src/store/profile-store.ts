...
export const useProfileStore = create<ProfileState>((set, get) => ({
  ...
  fetchProfiles: async () => {
    try {
      ...
      const all_profiles = response?.all_profiles ?? [];
      const last_played_profile_id = response?.last_played_profile_id ?? null;
      ...
      set({ profiles: [], error: i18n.t('profiles.errors.load_failed'), loading: false });
    }
  },
